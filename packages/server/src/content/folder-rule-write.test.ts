...
describe('applyNestedFolderRulesUpsert', () => {
  ...
  test('writes arbitrary keys (open-shape, like a doc)', () => {
    const result = applyNestedFolderRulesUpsert({
      projectDir,
      rules: [
        {
          match: 'specs/**',
          frontmatter: { title: 'Specs', status: 'draft', owner: 'tim', priority: 1 },
        },
      ],
    });
    ...
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    const content = readFileSync(join(projectDir, 'specs', '.ok', 'frontmatter.yml'), 'utf-8');
    expect(content).toContain('title: Specs');
    expect(content).toContain('status: draft');
    expect(content).toContain('owner: tim');
    expect(content).toContain('priority: 1');
  });

  test('clearing any key removes it while preserving the rest', () => {
    const fmDir = join(projectDir, 'specs', '.ok');
    mkdirSync(fmDir, { recursive: true });
    writeFileSync(join(fmDir, 'frontmatter.yml'), 'title: Specs\nstatus: draft\n', 'utf-8');

    const result = applyNestedFolderRulesUpsert({
      projectDir,
      rules: [{ match: 'specs/**', frontmatter: { status: null } }],
    });
