...
describe('delete_document MCP tool', () => {
  ...
  test('emits route-only previousPreviewUrl for the deleted docName when a preview source resolves', async () => {
    ...
    expect(result.structuredContent).toMatchObject({
      ok: true,
      deletedDocNames: ['old-page'],
      previousPreviewUrl: '/#/old-page',
    });
  });
});

describe('delete_document — batch (docNames)', () => {
  test('deletes each doc and reports per-doc results', async () => {
    const { server, registrations } = createCapturingServer();
    ...
    globalThis.fetch = (async (_input: RequestInfo | URL, init?: RequestInit) => {
      ...
      return new Response(JSON.stringify({ ok: true, deletedDocNames: [body.path] }), {
        status: 200,
        headers: { 'Content-Type': 'application/json' },
      });
    }) as typeof fetch;
    ...
    const result = await tool.handler({ docNames: ['a', 'notes/b', 'c'] });

    expect(result.isError).toBeUndefined();
    expect(deletedPaths.sort()).toEqual(['a', 'c', 'notes/b']);
    const s = result.structuredContent as {
      ok: boolean;
      documents: Array<{ docName: string; ok: boolean }>;
    };
    expect(s.ok).toBe(true);
    expect(s.documents).toHaveLength(3);
    expect(s.documents.every((d) => d.ok)).toBe(true);
    expect(result.content[0]?.text).toContain('3/3 deleted');
  });
  ...
  test('a per-doc failure surfaces without blocking the rest', async () => {
    ...
    globalThis.fetch = (async (_input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (body.path === 'missing') {
        return new Response(JSON.stringify({ ok: false, error: 'file does not exist' }), {
          status: 404,
          headers: { 'Content-Type': 'application/json' },
        });
      }
    ...
    const result = await tool.handler({ docNames: ['ok-doc', 'missing'] });
    ...
    expect(s.documents.find((d) => d.docName === 'ok-doc')?.ok).toBe(true);
    expect(s.documents.find((d) => d.docName === 'missing')?.error).toContain(
      'file does not exist',
    );
  });

  test('docName and docNames together is rejected', async () => {
    const { server, registrations } = createCapturingServer();
    register(server, makeDeps('http://localhost:4321'));
    const tool = getRegisteredTool(registrations, 'delete_document');

    const result = await tool.handler({ docName: 'a', docNames: ['b'] });
