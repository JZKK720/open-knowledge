...
import { register } from './edit-frontmatter.ts';
...
describe('edit_frontmatter — MCP tool over real CRDT path', () => {
  let harness: Harness;
  ...
  test('emits start-ui warning when no UI is attached + no preview URL resolves', async () => {
    ...
    const structured = result.structuredContent as {
      warning?: { action: string; previewUrl: unknown; message: string; autoOpen: boolean };
    };
    ...
    expect(structured.warning?.autoOpen).toBe(true);
  });
  ...
  test('start-ui warning carries autoOpen=false when user disabled it', async () => {
    ...
    register(server, {
      serverUrl: harness.baseUrl,
      config: ConfigSchema.parse({ appearance: { preview: { autoOpen: false } } }),
      resolveCwd: async () => harness.projectDir,
    });
    ...
    expect(result.isError).toBeUndefined();
    const structured = result.structuredContent as {
      warning?: { action: string; previewUrl: unknown; autoOpen: boolean };
    };
    expect(structured.warning?.action).toBe('start-ui');
    expect(structured.warning?.previewUrl).toBeNull();
    expect(structured.warning?.autoOpen).toBe(false);
  });

  test('attach-preview-once warning carries autoOpen=false when user disabled it', async () => {
    bindTestUiLock(harness.projectDir);
    await seedDoc('test-doc', '---\ntitle: My Doc\n---\n# Body\n');
    ...
    expect(structured.warning?.action).toBe('attach-preview-once');
    expect(structured.warning?.previewUrl).toBe('/#/test-doc');
    expect(structured.warning?.autoOpen).toBe(false);
  });
});
