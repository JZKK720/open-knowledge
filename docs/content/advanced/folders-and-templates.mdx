...
```text
write_document({ docName: "posts/launch", template: "blog-post" })
```
...
A folder only gets a `.ok/` directory once you give it a description or its first template; folders with nothing special stay plain. See the [Configuration reference](/docs/reference/configuration#folder-properties) for the file formats, and the [MCP reference](/docs/reference/mcp) for the `folder_config` and `write_document` tools that set the same things from an AI agent.
...
- [MCP reference](/docs/reference/mcp): the `write_document` tool and templates.
