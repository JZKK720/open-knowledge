...
- **Show all files** is on by default: it bypasses every filter (both client and server) and surfaces every file on disk under the content directory. Turn it off to scope the tree to indexed / linked content, with your `.gitignore` / `.okignore` honored.
