...
| `appearance.sidebar.showAllFiles` | boolean | `true` | project-local | On by default: the sidebar surfaces every file on disk under the content directory (synthetic system docs stay hidden), bypassing every ignore filter — both the client dot-segment drop and the server's `.gitignore` / `.okignore` / built-in exclusions. Set to `false` to scope the tree to indexed / linked content. Toggled from the sidebar's right-click menu or the macOS **View → Show All Files** item. |
